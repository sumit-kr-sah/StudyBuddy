...
router.post("/session/stop", authenticateToken, async (req, res) => {
  try {
    ...
    if (startTimeDate > endTime) {
      return res
        .status(400)
        .json({ message: "Start time cannot be in the future" });
    }
    ...
    if (duration < 1000) {
      return res
        .status(400)
        .json({ message: "Session too short (minimum 1 second)" });
    }

    if (duration > 24 * 60 * 60 * 1000) {
      return res
        .status(400)
        .json({ message: "Session too long (maximum 24 hours)" });
    }
    ...
    try {
      ...
      return res
        .status(500)
        .json({ message: "Failed to record study session" });
    }
        ...
        try {
          for (const achievement of newAchievements) {
            try {
              ...
              console.error(
                `Failed to send email for achievement ${achievement}:`,
                emailError
              );
            }
          }
          ...
          console.error(
            "Failed to send achievement notifications:",
            emailBatchError
          );
        }
      });
...
router.get("/sessions", authenticateToken, async (req, res) => {
  try {
    ...
    const sessions = user.studySessions.sort(
...
router.delete("/session/:sessionId", authenticateToken, async (req, res) => {
  try {
    ...
    if (session.startTime >= weekStart) {
      user.weeklyStudyTime = Math.max(
        0,
        user.weeklyStudyTime - session.duration
      );
    }

    if (session.startTime >= monthStart) {
      user.monthlyStudyTime = Math.max(
        0,
        user.monthlyStudyTime - session.duration
      );
    }
...
router.get("/today", authenticateToken, async (req, res) => {
  try {
    ...
    const todayStudyTime = user.studySessions
      .filter(
        (session) => session.startTime >= today && session.startTime < tomorrow
      )
      .reduce((total, session) => total + session.duration, 0);

    const todaySessions = user.studySessions.filter(
      (session) => session.startTime >= today && session.startTime < tomorrow
    ).length;
  ...
  try {
    ...
    for (let i = 6; i >= 0; i--) {
      ...
      const dayStudyTime = user.studySessions
        .filter(
          (session) => session.startTime >= date && session.startTime < nextDate
        )
        .reduce((total, session) => total + session.duration, 0);
